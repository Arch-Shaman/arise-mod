...
function Spring.Utilities.GetHumanName(ud, unitID)
	...
	if unitID then
		local unitLevel = (Spring.ValidUnitID(unitID) and Spring.GetUnitRulesParam(unitID, "unitlevel"))
		...
		if unitLevel then
			return "Lv. " .. (unitLevel or "?") .. " " .. (WG.Translate ("units", name_override .. ".name") or ud.humanName)
		end
	end
