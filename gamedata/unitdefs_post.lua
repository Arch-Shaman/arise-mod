...
-- Arise: Create clonedefs for factories. --
...
do
	...
	local function CreateNewDef(def, level)
		local newDef = deepcopy(def)
		local newName = def.unitname .. "_" .. level
		newDef.unitname = newName
		newDef.customParams.factoryLevel = level
		if rules.payForNewFactories then
			local cost = def.buildcostmetal * (rules.costFactor * (level - 1))
			newDef.buildcostmetal = cost
			newDef.buildcostenergy = cost
			newDef.buildtime = cost
		end
	...
	for _, name in pairs(allFactories) do
		for i = 1, rules.maxLevel do
			if i > 1 then
				local newDef = CreateNewDef(UnitDefs[name], i)
				local newName = newDef.unitname
				UnitDefs[newName] = newDef -- let's see about injecting it here. No idea if this actually works
			else -- if we're level one do this.
				UnitDefs[name].customparams.morphto = name .. "_2"
				UnitDefs[name].customparams.morphtime = math.min((UnitDefs[name].buildcostmetal * rules.costFactor)/rules.upgradeBuildPower, rules.upgradeTimeMax)
			end
		end
	end
end
